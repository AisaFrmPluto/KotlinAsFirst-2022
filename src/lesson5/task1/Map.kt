...
fun mergePhoneBooks(mapA: Map<String, String>, mapB: Map<String, String>): Map<String, String> {
    val result = subtractOf(mapA.toMutableMap(), mapB.toMutableMap())
    for ((key, value) in mapB)
        if (key !in result) result[key] = value
        else result[key] += ", " + mapB[key]
    return result
}
...
fun averageStockPrice(stockPrices: List<Pair<String, Double>>): Map<String, Double> {
    ...
    for ((first, second) in stockPrices) {
        map[first] = if (first in map) map.getOrDefault(first, listOf(second)) + second
        else map.getOrDefault(first, listOf(second))
    }
...
fun findCheapestStuff(stuff: Map<String, Pair<String, Double>>, kind: String): String? {
    ...
    for ((key, value) in stuff)
        if (((price == -1.0) || (price > value.second)) && (stuff[key]?.first == kind)) {
            price = value.second
            result = key
        }
    return result
 ...
 *   ) -> emptySet()
 */
fun bagPacking(treasures: Map<String, Pair<Int, Int>>, capacity: Int): Any {
